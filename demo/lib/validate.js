...
function validateField(field, value) {
    const make = (code, message, detail, severity = "error") => ({
        field: field.name,
        label: field.label,
        code,
        severity,
...
function validateString(field, value, c, make) {
...
function validateNumber(field, value, c, make) {
...
function validateDate(field, value, make) {
    if (typeof value !== "string") {
...
function validateEmail(field, value, c, make) {
    if (typeof value !== "string") {
        return [make("invalid_type", `${field.label} must be text.`, { expected: "string", received: typeName(value) })];
    }
...
function validatePhone(field, value, c, make) {
    if (typeof value !== "string") {
        return [make("invalid_type", `${field.label} must be text.`, { expected: "string", received: typeName(value) })];
    }
    const issues = validateString(field, value, c, make);
...
function validateChoice(field, value, make) {
    if (typeof value !== "string") {
...
function validateMultiChoice(field, value, c, make) {
    if (!Array.isArray(value)) {
...
function validateFile(field, value, c, make) {
...
function findUnknownFields(definition, record) {
